...
- **`apply`/data query response is chunked NDJSON, not one merged array**: the streaming operator (`archiver/stream.py`) loads data in time-chunks of `(5 days)/num_streams` seconds (floor 300s, `DATA_DAYS=5`), and `JsonConsumer.write` (`archiver/consumers.py`) emits each chunk as a separate `json.dumps(...)` + `\r\n`. So one sensor's series spanning multiple chunks appears as MULTIPLE objects with the SAME uuid (consecutive time-slices); total objects ≈ sensors × chunks. Clients must group by uuid and order by timestamp to stitch a full series. A short/single-stream query fits in one 5-day chunk so it looks like a single object — misleading.
